...
class PageGroup:
    ...
    def _normalize_label(self, label: str) -> str:
        url = "".join(char.lower() for char in label if char.isascii()).strip().replace(" ", "-")
        url = parse.quote(label)
        return url

    # def _normalize_label_(self, label: str) -> str:
    #     # url = ""
    #     # url = "".join(char.lower() for char in label if char.isascii()).strip().replace(" ", "-")
    #     # print('url', url)
    #
    ...
    #     for char in label:
    #         if char.isascii():
    #             url = url.join(char.lower())
    #         if "u4e00" <= char <= "u9fa5":
    #             url = url.join(parse.quote(char))
    #
    #     url.strip().replace(" ", "-")
    #     return url
